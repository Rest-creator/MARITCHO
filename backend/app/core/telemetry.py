...
class JSONLogFormatter(logging.Formatter):
    """One JSON object per line: timestamp, level, logger, message, trace/span
    IDs (populated by TraceIDFilter), and exception info when present.

    Replaces the previous plain formatted-text-with-embedded-IDs output —
    BACK-002. Scoped to this application's own logger ("maricho-api");
    uvicorn's built-in access/error logging keeps its own default
    formatting, unconfigured here.
    """
