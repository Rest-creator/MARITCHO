...
| **RSK-002** | **Connection Starvation:** Backend crashes due to maxing out database connections during a network reconnection burst. | High | High | Enforce strict asynchronous connection pooling in SQLAlchemy (`pool_size=20`, `max_overflow=10`). |
